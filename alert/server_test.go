...
// See https://golang.org/src/net/http/httptest/recorder_test.go for more examples
...
func TestApp(t *testing.T) {
	...
	hasNoAlerts := func() checkFn {
		return func(r *http.Response, body string) error {
			...
			if len(nodes) != 0 {
				return fmt.Errorf("len(alerts)=%d; want len(elerts)=0", len(nodes))
